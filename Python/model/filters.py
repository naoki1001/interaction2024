...
class RawDataForPoseEstimation:
    ...
    def forward(self, u, z=None, dt=0.005):
        ...
        if z is None:
            ...
            self.q = convert_quaternion(R.from_euler('zxy',[*self.get_euler(u=u)], degrees=False).as_quat())
        else:
    ...
    def get_next_state(self, u, z=None, dt=0.005):
        ...
        if z is None:
            ...
            self.v = self.v + dt * acc_g
            #self.q = convert_quaternion(R.from_euler('zxy',[*self.get_euler(u=u)], degrees=False).as_quat())
            self.q = convert_quaternion(R.from_euler('zxy',[*self.ekf.x.reshape(3)], degrees=False).as_quat())
        else:
            rotation_matrix = R.from_quat(convert_quaternion(z[3:])).as_matrix()
            acc_g = convert_axis_to_unity(rotation_matrix @ acc - self.g)
            self.p = z[:3].reshape(-1, 1)
    ...
    def get_euler(self, u, dt=0.005):
        ...
        gyro = u[3:]
        r = R.from_quat(convert_quaternion(self.q))
        euler = (R.from_euler('zxy',[*(gyro * dt)], degrees=False) * r).as_euler('zxy')
...
class IMMUKFForPoseEstimation(nn.Module):
    def __init__(self, dim_x=10, dim_z=3, dim_u=6, dt=0.005):
        super(IMMUKFForPoseEstimation, self).__init__()
        self.dim_x = dim_x
        self.dim_z = dim_z
        self.dim_u = dim_u
